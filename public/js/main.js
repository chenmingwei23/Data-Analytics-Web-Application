...
$(document).ready(function(){
	$.get('/spa/getTotalUser', null, function (data) {
		// Can't set a big value for the user array, as autocomplete can't support that much information
        for (var i = 0; i < 3000; i++) {
            users.push(data[i]._id);
        }
        console.log(users);
        $( "#gerUserName" ).autocomplete({
            source: users
        });
    });
    ...
    $("#getAuthor").click(function (e) {
    	$("#userCreated").empty();
    	$("#getUserRevision").empty();
    	
        var parameters = {userName: $('#gerUserName').val()};
        
        $.getJSON('/spa/getUserRevisions', parameters, function (data) {
            $("#userCreated").append("Revisions by this author are here, you can choose one to view more information");
        	for (var i = 0;i<data.length;i++) {
            	$("#getUserRevision").append("<option value='"+data[i]._id+"'>"+"Revision name is "+data[i]._id+", revision number is "+data[i].revisionCount+"<br>");
            }        	
        });
        $("#getUserRevisionButton").click(function (e) {
        	$("#getRevisionTime").empty();
        	
        	var para = {userName:parameters.userName, revisionName: $("#getUserRevision").val()};
        	$.getJSON('/spa/getUserRevisionsTime', para, function (data1) {
        		//console.log(data1);
            	for (var i = 0;i<data1.length;i++) {
            		console.log(data1[i]);
                	$("#getRevisionTime").append("Edit time: "+data1[i].timestamp+"<br>");
                }        	
            });
   	 	});
